...
public class IOSAPMSDK : MonoBehaviour
{
    ...
    [DllImport("__Internal")]
    internal extern static float GetProcessUsedMemory();      //返回当前进程使用的内存
    [DllImport("__Internal")]
    internal extern static float GetDeviceTotalMemory();      //返回设备总的内存
    [DllImport("__Internal")]
    internal extern static float GetDeviceFreeMemory();       //返回设备剩余的内存
    [DllImport("__Internal")]
    internal extern static float GetDeviceUsedMemory();       //返回设备已经使用的内存
    [DllImport("__Internal")]
    internal extern static double GetBatteryLevel();          //返回当前电池电量
    [DllImport("__Internal")]
    internal extern static float GetDeviceAvailableMemory();  //该接口返回的数值等于设备总内存减去脏内存的大小,仅适用于ios 13.0以上的版本
